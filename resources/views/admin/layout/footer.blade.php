<footer class="main-footer">
    <div class="pull-right hidden-xs">
        Powered by Done-Right Systems - 08038883919.
    </div>
    <strong>Copyright &copy; <script>
            var y=new Date(); document.write(y.getFullYear());
        </script> PISS </strong> All rights
    reserved.
</footer>
...
<script>
    //$(document).ready(function(){
        $('.add_more').on('click',function(){
            ...
            var tr='<tr><td><select class="form-control product_id" name="product_id[]">'+ product +'</select></td>'+
                '<td><input type="text" name="unitprice[]" id="unitprice" class="form-control unitprice" readonly></td>'+
                '<td><input type="text" name="quantity[]" id="quantity" class="form-control quantity"></td>'+
                '<td><input type="text" name="totalamount[]" id="totalamount" class="form-control totalamount" readonly></td>'+
                '<td><a href="#" class="btn btn-danger btn-sm delete"><span class="fa fa-times"></span></a></td>'+
                '</tr>'
        ...
        $('.addMoreProduct').delegate('.product_id,.quantity','change',function(){
            var tr=$(this).parent().parent();
            var unitprice=tr.find('.product_id option:selected').attr('data-price');
            tr.find('.unitprice').val(unitprice);
            var quantity=tr.find('.quantity').val() - 0;
            var unitprice=tr.find('.unitprice').val() - 0;
    ...
    $('.addMoreProduct').delegate('.product_id,.quantity','keyup',function(){
            var tr=$(this).parent().parent();
            var unitprice=tr.find('.product_id option:selected').attr('data-price');
            tr.find('.unitprice').val(unitprice);
            var quantity=tr.find('.quantity').val() - 0;
        ...
        function printReceipt(print){
            var data='<input type="button" id="printPage" value="Print Receipt" onClick="window.print()" class="printPage" style="display:block; width:100%; border:none; background-color:#005674; color:#fff;padding:14px 28px;font-size:16px;cursor:pointer;text-align:center;"';

            data +=document.getElementById(print).innerHTML;

            myReceipt=window.open("","myWindow","left=150, top=130, width=400, height=450");
            myReceipt.screenX=0;
            myReceipt.screenY=0;
